...
NSString * const UncaughtExceptionHandlerAxyessesKey = @"UncaughtExceptionHandlerAxyessesKey";
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    NSString *message = [NSString stringWithFormat:@"reason:%@\n",exception.reason];
    if (exception.userInfo) {
        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:exception.userInfo options:NSJSONWritingPrettyPrinted error:nil];
        message = [message stringByAppendingFormat:@"userInfo:%@\n",[[NSString alloc]initWithData:jsonData encoding:NSUTF8StringEncoding]];
    }
    [XYUtils showAlertWithTitle:@"崩溃啦" message:message cancelButtonTitle:@"退出" otherButtonTitles:@"继续" completeBlock:^(NSInteger integer) {
        if (integer == 0) {
            self.dismissed = YES;
        }
    }];
...
#pragma clang diagnostic pop
    ...
    [userInfo setObject:callStack forKey:UncaughtExceptionHandlerAxyessesKey];
