...
@implementation XYHorizontalScrollView
...
- (void)reloadData {
    ...
    // 新数据count < 旧数据count
        ...
        // 移动offset.x到新数据maxX
        if (offsetX+SCREEN_WIDTH >= resetMaxX) {
            // 设置新的visibleItems
            ...
            resetMaxX = [self addItemsUsingBlock:^{
                ...
                for (NSInteger i = resetCount-1; i > -1; --i) {
                    ...
                    if (i < resetCount-1) {
                        isOverScreenWidth = rect.origin.x <  maxX - SCREEN_WIDTH - overItemWidth;
                    }
                    // 超过屏幕可显示范围break
                    ...
                    // 获取Cell
                    XYHorizontalScrollViewItem *item = [weakSelf editVisibleItemPlaceWithIndex:i originX:rect.origin.x];
                    if (item) {
                        // 加入到visibleItems
        ...
        // scrollView的offset.x < 新数据maxX
            ...
            [self addItemsUsingBlock:^{
                ...
                for (NSInteger i = resetIndex; i < weakSelf.count; ++i) {
                    ...
                    isOverScreenWidth = (maxX > SCREEN_WIDTH + overItemWidth);
...
- (XYHorizontalScrollViewItem *)dequeueItemWithIdentifer:(NSString *)identifer {
    ...
    XYHorizontalScrollViewItem *item = tempSet.anyObject;
    return item;
}
...
- (void)setupData {
    ...
    for (NSInteger i = 0; i < _count; ++i) {
        ...
        if (!isOverScreenWidth) {
            ...
            XYHorizontalScrollViewItem *item = [self itemWithIndex:i];
            if (item) {
...
- (void)tapItem: (UITapGestureRecognizer *)tapGst {
    ...
    for (XYHorizontalScrollViewItem *item in _visibleItems) {
        item.isSelected = NO;
    ...
    if ([tapGst.view isKindOfClass:[UIView class]]) {
        ...
        XYHorizontalScrollViewItem *item = (XYHorizontalScrollViewItem *)tempView;
        
        if ([item isKindOfClass:[XYHorizontalScrollViewItem class]]) {
            item.isSelected = YES;
 ...
 *  @return item
 */
- (XYHorizontalScrollViewItem *)itemWithIndex:(NSInteger)index {
    XYHorizontalScrollViewItem *item = nil;
 ...
 */
- (XYHorizontalScrollViewItem *)editVisibleItemPlaceWithIndex:(NSInteger)index originX:(CGFloat)originX {
    XYHorizontalScrollViewItem *item = [self itemWithIndex:index];
...
- (CGFloat)addItemsUsingBlock:(void(^)(void))block {
    ...
    if (_visibleItems.count > 0) {
        CGFloat offsetX = CGRectGetMaxX([_visibleItems.lastObject frame]) - SCREEN_WIDTH;
        if (offsetX < 0) {
            offsetX = 0;
        }
        return offsetX;
 ...
 */
- (void)addReuseItem:(XYHorizontalScrollViewItem *)reuseItem {
...
- (void)srollHandle:(UIScrollView *)scrollView {
    ...
    XYHorizontalScrollViewItem *topItem = _visibleItems.firstObject;
    XYHorizontalScrollViewItem *lastItem = _visibleItems.lastObject;
    //3 滚动方向处理
    // 最左边和最右边的情况
    if (offsetX < 0 || offsetX+SCREEN_WIDTH > scrollView.contentSize.width) {
        // 不处理
    }
    // 往右滚动,第一个item已经在屏幕消失
        ...
        while (nextIndex < _count) {
            ...
            XYHorizontalScrollViewItem *nextItem = [self editVisibleItemPlaceWithIndex:nextIndex originX:maxX];
            ...
            nextIndex++;
            if (maxX > offsetX + SCREEN_WIDTH + width) {
                break;
            }
        }
    ...
    // 往左滚动，最后一个item已经在屏幕消失
    else if (offsetX < _offset && offsetX+SCREEN_WIDTH <= lastItem.frame.origin.x) {
        ...
        while (previousIndex > -1) {
            ...
            XYHorizontalScrollViewItem *previousItem = [self editVisibleItemPlaceWithIndex:previousIndex originX:originX - width];
